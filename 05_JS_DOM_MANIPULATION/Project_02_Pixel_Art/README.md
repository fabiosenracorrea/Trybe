...
This project got a **100%/100%** grade and can be checkout entirely on [here](https://github.com/tryber/sd-06-project-pixels-art/pull/85).

#### Comments

The challenge of this project was to a project that was more than just what was requested. To try and give it a little more room so it could provide a better experience to whoever would use it.

Because of the constraints on the project (needing to only have black and 3 other colors on it's base base), i've decided to get creative: since the colors required to be random every time, i added a button, animated with CSS, to allow the user to create another random color (up to 40 total). The only adjustment made here in comparison to the linked project is the order of the elements on screen.

Since the main focus on the pixel art should be the actual drawing, i decided to leave this page 'as empty' as possible, and i've actually used it a few times to relax and try to come up with some nice drawing.

This page did NOT receive responsivity support to size things down, but it should be a relative easy task.
