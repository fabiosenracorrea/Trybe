## Project 01 - React Hooks to the test: useTable

The first project of Trybe's 18th module, centered using React Hooks to it's full potential;
...
Os dados recebidos da API devem ser salvos num campo chamado `data` do contexto e é daí que a tabela deve lê-los. A requisição deve ser feita num componente separado do componente da tabela.
...
This project got a **100%/100%** grade and can be checkout entirely on [here](https://github.com/tryber/sd-06-project-starwars-datatable-hooks/pull/58).


#### Comments

As highly anticipated, React Hooks make React *so much better*. Provided that you know well enough of the crucial ones (useState, useEffect, useCallback, useMemo, useContext, useRef and your own custom ones), it brings the functional components well above classes.

Knowing how to effectively use them will definitely make you stand out comparing to those who don't.

As a highlight of the project, i'd like to point out 2 pieces of code:

1. The API call that consumes the entirety of the planets' database to construct the array that will be used on the application
...
async function fetchPlanets() {
  ...
  const maxPages = Math.ceil(count / resultsPerPage) - 1; // because of the initial
  const PAGE_JUMP = 2;
...
2. The whole logic behind the `usePlanets` hook created. It manages so much for the application and applying a lot of useMemos and useCallbacks, taking full advantage of those awesome hooks!

[check the hole hook file out](./src/hooks/planets.jsx)
