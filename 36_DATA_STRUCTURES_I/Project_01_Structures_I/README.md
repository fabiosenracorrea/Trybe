## Project 01 - Data Structures I - Hashes, Lists & Sets

The first project of Trybe's 38th module, in which we dive in a little on **Data Structures**
...
This project got a **100%/100%** grade and can be checkout entirely on [here](https://github.com/tryber/sd-06-restaurant-orders/pull/28).

#### Comments

Dealing with data structures is something every developer will do EVERY DAY in his life. Not only having the ability to deal with the provided structures, it's crucial to know how to properly **create and parse** the data you are working with. Sometimes your data will come dirty and not on an ideal format to properly work with it. Sometimes they may come, but there's still performance implementations that could be made. Moreover, there will be times you just want it in a specific format, so you have to transform and make it that way. And that is applicable anywhere, from the regular frontend with vanilla JS or the more complex React applications to a small CRUD python server or a huge micro-service application in Node. There's no escape from it, so you better get it right!
