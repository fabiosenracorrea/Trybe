## Project 01 - NodeJS: Crush Manager

The first project of Trybe's 26th module, centered around simple CRUD operations on an Express server using the filesystem (fs) from Node.
...
This project got a **100%/100%** grade and can be checkout entirely on [here](https://github.com/tryber/sd-06-crush-manager/pull/62).

#### Comments

NodeJS has come a long way. Creating an Express server with it couldn't be easier. This project is a simple representation of how one can perform CRUD operations on that kind of server. To top it off, it makes usage of Node's great filesystem, which makes it easy to manage files within our project's directory.

Although it was not required, I decided to develop this project using MSC architecture, applying a little bit of SOLID concepts.

There are some validations that could easily be replaced by middlewares, however.
