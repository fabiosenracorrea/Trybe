...
This project got a **100%/100%** grade and can be checkout entirely on [here](https://github.com/tryber/sd-06-project-trivia-react-redux/pull/271).

### Playing the game

If you want to test this game for yourself, it's pretty easy!

1. Make sure you have [NodeJS](https://nodejs.org/en/) or [Yarn (classic)](https://classic.yarnpkg.com/lang/en/) installed on your machine;
2. Clone this repository;
3. Navigate to the cloned directory and run `npm install` or `yarn`;
4. After the installation, simply run `npm start` or `yarn start`;
5. The game will open on your browser. Have fun!

#### Comments

As seen before, Redux helps a lot to manage the global state of our application, and when designing a game, which is usually passes information from one place to another, exactly what Redux needs to shine.

As a highlight of the project, i'd like to comment on logic created to generate the correct API URL, based on the user's preferences, using our `config` object to generate the `query params` needed.
