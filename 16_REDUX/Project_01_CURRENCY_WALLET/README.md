...
This project got a **100%/100%** grade and can be checkout entirely on [here](https://github.com/tryber/sd-06-project-trybewallet/pull/59).


#### Comments

As widely seen on this module, Redux can really help out on managing our application information. It's clear example is how we can save the logged user's info there to quickly obtain it instead of having to go to the localStorage every time. But there's a lot of other situations we can also use it pretty effectively, as shown on this application.

As far as context-API comparison, i'll wait until a project is done using it to be fair on the analysis. What's certain is that, without it, Redux would probably be one of your best choices when managing global states on your applications.

As a highlight of the project, i'd like to comment on the use of [Thunk](https://github.com/reduxjs/redux-thunk) to handle every API call effectively, such as:
