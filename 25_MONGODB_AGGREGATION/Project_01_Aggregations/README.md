## Project 01 - MongoDB: DataFlights

The first project of Trybe's 23th module, centered around simple CRUD operations on Mongo.
...
4. Na raiz do diretório do projeto, execute o seguinte comando que fará a restauração da base de dados `aggregations`:
    ```sh
...
This project got a **100%/100%** grade and can be checkout entirely on [here](https://github.com/tryber/sd-06-mongodb-aggregations/pull/49).


#### Comments

Aggregation queries is where we can get our data to the next level. The amount of complex operations we can perform relatively easily using the aggregation pipeline is awesome.

If you come from a SQL usage, you will see nothing (not even joins) are out of question here, and a lot of conclusions can be withdrawn from your database provided you know which operators to use.
